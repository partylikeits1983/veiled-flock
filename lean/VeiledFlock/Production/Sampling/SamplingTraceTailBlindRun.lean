import VeiledFlock.Production.Sampling.SamplingTraceTailBlindOracle
...
theorem runBlindGrinding_of_not_globalBad
  ...
  have horacle : ∀ offset : Fin maxBlindTrials,
      (∀ prior : Fin maxBlindTrials, prior.val < offset.val →
        ¬ blindGrindingGood shape (grindAnswers prior)) →
  ...
  have hexists : ∃ offset : Fin maxBlindTrials,
      blindGrindingGood shape (grindAnswers offset) := by
  ...
  have hstateAnswer : oracle (powStatePoint transcript) = state := by
    ...
    have hquery : rawQuery shape causalSecret completion witness coins site before =
        some (powStatePoint before.transcript) := by
    ...
    calc
      oracle (powStatePoint transcript) =
          oracle (powStatePoint before.transcript) := by
        exact congrArg oracle (congrArg powStatePoint htranscript.symm)
