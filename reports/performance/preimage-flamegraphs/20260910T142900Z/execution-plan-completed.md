...
Date: 2026-09-10. Status: execution completed with a documented deviation from
the planned AC-only power protocol. All 44 accepted recordings, four baseline
CSVs, corrected primary SVGs, and the local report are available; no commit or
push has occurred.

Deliverables: [report](../../reports/performance/preimage-flamegraphs/20260910T142900Z/report.md),
[28-SVG gallery](../../reports/performance/preimage-flamegraphs/20260910T142900Z/index.html),
[artifact inventory](../../reports/performance/preimage-flamegraphs/20260910T142900Z/artifacts.json),
and [completion record](../../reports/performance/preimage-flamegraphs/20260910T142900Z/complete.json).
...
## Execution amendment: resumed measurements and frame-label correction

The completed run is
[`20260910T142900Z`](../../reports/performance/preimage-flamegraphs/20260910T142900Z/manifest.json)
on `chore/preimage-flamegraphs`. Before the pause, it collected all 28 primary
captures, eight smoke captures, the four 64-hash endpoint repeats, and the
normal-before and symbol baselines. The first 4,096-hash FLOCK-prove repeat
(`flock-prove-4096/repeat-001`) was interrupted and remains failed evidence.
It does not qualify as a completed repeat. Resumption completed the four large
endpoint repeats in fresh `repeat-002` directories. The accepted inventory is
28 primary captures, eight smoke captures, and eight endpoint repeats: 44
recordings in total. Frozen-source checks, including `make test`, passed;
all 44 accepted recordings were reconstructed from raw XML and their corrected
SVGs rendered successfully. The four baseline CSVs and final report were
generated, and the runner exited successfully.

**Observed power-protocol deviation:** all primary capture boundary
observations report Battery Power, declining from 27% to 11%. The retained
64-hash repeats ended at 8%, with an early battery warning. The resumed
4,096-hash repeats ran on AC power while charging, at 46%–51%. Thus the planned
AC-only condition was **not met** for the primary episode. The outputs describe
these observed conditions; they must not be presented as a controlled AC-only
benchmark. The missing primary-episode closing baseline and changed power
conditions limit timing comparisons across episodes.

For this run, amend the single-controller sequence in sections 4 and 5 as
follows. Preserve the original frozen source **A**, its workload and helper
executables, recorded Cargo/Rust/profiler identities, and all accepted captures.
The `--resume <manifest>` entry point prepares a separate frozen controller
source **B**, validates and builds that controller in separate output
directories, and records its identity in `resume.json`. Before any resumed
measurement, reverify A and the original executable/tool hashes. Run workloads,
helpers, and the profiler against their original registered identities; B
changes orchestration only and cannot retrospectively redefine A's captures.
If those identities or the measurement policy cannot be preserved, use a new
measurement run instead of appending results to this one.
...
Finish all controller builds and checks before settling for measurement.
Retain the first 28 primaries and four 64-hash repeats, then run only the four
missing 4,096-hash repeats with fresh, exclusive attempt IDs. Preserve the
interrupted `repeat-001` directory; a retry must use a new directory. Collect
`baseline-resume-before.csv` before the resumed repeats and `baseline-after.csv`
after them, using the unchanged normal benchmark executable and environment
policy. Record the resumed host/power observations and pause in `resume.json`.

These are **two measurement episodes**. The original normal-before and symbol
baselines describe the first episode. The resume-before and final-after
baselines bracket only the four resumed large endpoint repeats. There is no
immediate closing baseline for the primary episode: original-before versus
final-after deltas are cross-session changes, not evidence of continuous
drift across an uninterrupted primary series. Label 64-hash repeats as
same-episode comparisons and 4,096-hash repeats as cross-session comparisons.
The report retains this limitation even though all required endpoint captures
have now succeeded.

Inspection also found an upstream folded-stack formatting defect: demangled
Rust array types such as `[u8; 64]` contain ASCII semicolons, which the folded
format interprets as frame separators. After the final baseline, reconstruct
each accepted attempt's stacks from its archived XML frame/backtrace references,
demangle frame names, and display embedded semicolons as `；` before joining
frames with ASCII `;`. Regenerate SVGs and hotspot aggregates from those
corrected stacks. Preserve original folded stacks, SVGs, previews, and analysis
alongside unchanged raw XML, traces, harness metadata, and recorder statuses.
Record the revised reporting source/dependencies, executable identities, input
and output checksums, and original-artifact mapping in `report-processing.json`,
separately from A and B. The sample denominator remains usable backtrace row
occurrences, not the XML weight column.

Controller/report corrections do not authorize workload changes during this
run. Do not build reporting tools or regenerate large artifacts during active
captures or baselines. All eight accepted endpoint repeats, four baselines,
corrected and rendered primary SVGs, artifact/provenance records, and the report
have now been produced. **No commit or push is authorized without the user's
explicit approval.**
...
baseline-resume-before.csv          # resumed runs: before the resumed endpoint repeats
resume.json                        # resumed controller identity and measurement episodes
report-processing.json             # derived-artifact correction and reporting provenance
...
7. For the resumed run, distinguish the two measurement episodes, show the
   resumed interval's controls, label endpoint repeat comparisons by episode,
   and state that the primary episode has no immediate closing baseline.
8. Explain the frame-label correction, retain links to original evidence, and
   distinguish report-processing identities from the measured source snapshot.
...
The completed artifact/check criteria below do not assert that the AC-only
condition was met. Its observed violation is documented in the execution
amendment and must accompany interpretation of the results.

- [x] Work is on the recorded branch with a conventional prefix; existing
      changes and branches are preserved.
- [x] No commit or push has occurred without explicit user approval for that
      action; the complete local results are ready for review.
- [x] All 28 primary SVGs are present, nonempty, valid XML, and render correctly.
- [x] Correct operation/size/protocol labels and actual worker stacks are checked.
- [x] Baseline proofs and all verification calls succeed; profiling proof checks
      and loop overhead are disclosed accurately.
- [x] Baselines, captures, and repeats use the same diagnostic/default-variable policy;
      custom Cargo target paths select the newly built Rust runner and helpers.
- [x] All diagnostic keys and default-policy overrides are absent as required;
      the harness reports the actual environment, including through xctrace.
- [x] Implementation checks passed before measurements; the uncommitted source
      snapshots existed before their final checks/builds, reconstruct their build
      inputs, and supplied the declared workload, controller, and profiler-build roles.
- [x] Every accepted executable pairs successful frozen-source validation and
...
- [x] Every published capture has matching harness and wrapper success metadata;
      interrupted, partial, stale, or failed attempts cannot qualify.
- [x] Archived traces survive recorder cleanup, and scratch/archive paths are
      distinct; smoke, primary, repeat, and retry attempts cannot overwrite each other.
- [x] Raw evidence, folded stacks, repeat captures, build provenance, and exact
      reproduction commands are retained.
- [x] Resumed work preserves A's workload/helper/tool identities, records the
      separately frozen and validated controller B, and labels both measurement
      episodes and the missing primary-episode closing baseline accurately.
- [x] Embedded frame-name semicolons are corrected from raw XML; original
      artifacts and separate report-processing provenance are preserved.
- [x] The report links every SVG, includes measured hotspot analysis and fresh
      baselines, and distinguishes the published table from current results.
- [x] No missing or unusable capture is presented as completed work.
