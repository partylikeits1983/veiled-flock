...
pub fn write_report(root: &Path) -> Result<()> {
    ...
    md.push_str("\nFull-ZK batches of 64 and 128 pad to 256 slots. Non-ZK uses smaller circuits and ad hoc PCS schedules below the registry floor. Serialized proof sizes exclude returned witness commitments, public digests, and bundle framing. CPU profiles do not establish proof size overhead.\n\n### Baseline drift and debug-symbol comparison\n\nDeltas are relative to the normal baseline before profiling. The two normal baselines bracket the capture series; the symbol baseline runs before the series. These are five-sample medians, without confidence intervals. A large delta warrants checking load/thermal observations and repeating baselines before attributing smaller differences to code.\n\n| Hashes | Protocol | Normal after prove / verify ms | After drift prove / verify | Symbols prove / verify ms | Symbol delta prove / verify | Bytes before min–max | Bytes after min–max | Bytes symbols min–max |\n| ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |\n");
    ...
    writeln!(
        md,
        "\nLargest absolute before/after timing delta: **{max_drift:.1}%**. Largest absolute symbol-build delta: **{max_symbol:.1}%**. These comparisons combine build effects and measurement variability; they do not isolate a causal debug-symbol effect. Exact data: [before](baseline-before.csv), [after](baseline-after.csv), [symbols](baseline-symbols.csv).\n"
    )?;
    ...
    for (index, (name, (mean, case, peak))) in scores.into_iter().take(5).enumerate() {
        let experiment = match name.as_str() {
            "allocation and copying" => {
                "Measure allocation counts and bytes for the leading stacks, then test buffer reuse in one API path. Re-run unchanged correctness checks, unprofiled medians, and the same endpoint captures to determine whether reduced copying or allocation changes latency."
            }
            "scheduling and synchronization" => {
                "Inspect worker wakeups and task granularity, then benchmark a controlled task-size change while preserving the dedicated verifier pool. Check both latency and total CPU activity; busy waiting or blocked stacks can distort a simple share comparison."
            }
            "transcript, hashing, and randomness" => {
                "Separate transcript hashing, Merkle hashing, and entropy acquisition using the recorded call paths. Benchmark batching or buffer reuse around the leading routine while preserving transcript bytes and fresh full-ZK randomness."
            }
            "field arithmetic and transforms" => {
                "Benchmark the leading arithmetic/transform kernel with representative dimensions, then test one layout or vectorization change. Require bit-exact results and full protocol tests before measuring end-to-end effects."
            }
            "sumchecks and constraint checks" => {
                "Inspect passes over witness and constraint arrays in the leading routine. Test reducing redundant traversal or temporary storage while preserving every public API witness and soundness check."
            }
            "PCS and Merkle work" => {
                "Measure commitment/opening substeps separately, then test a bounded data-layout or hashing-buffer change. Preserve PCS parameters, transcript contents, query schedules, and proof verification."
            }
            _ => {
                "Inspect the leading VEIL call path and its temporaries, then benchmark a localized buffer or traversal change. Preserve blinding, randomness, transcript domains, and all protocol validation."
            }
        };
        writeln!(
            md,
...
mod tests {
    ...
    fn capture_counts_rows_deduplicates_recursion_and_reconciles_folded() {
        ...
        fs::write(p.join("raw/time-profile.xml"),r#"<trace-query-result><node><row><weight>999999</weight><thread fmt="flock-verify"/><backtrace id="1"><frame id="1" name="flock_core::verifier::verify_core_inner"/><frame id="2" name="rayon_core::worker"/></backtrace></row><row><backtrace ref="1"/></row><row><backtrace ref="1"/></row><row><sentinel/></row></node></trace-query-result>"#).unwrap();
        fs::write(p.join("stacks.folded"),"rayon_core::worker;flock_core::verifier::verify_core_inner;flock_core::verifier::verify_core_inner 2\nrayon_core::worker;0x1234 1\n").unwrap();
