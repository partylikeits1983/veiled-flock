...
// Keep this generator identical to preimage_scaling so the workload matches
// the published benchmark without changing that example's command or CSV.
...
mod tests {
    ...
    fn verify_rotates_corpus_without_reproving_inside_loop() {
        ...
        let measurement = measure(
            ...
            |_, proof| {
                let index = verified.get();
                if index >= VERIFY_CORPUS_SIZE {
                    assert_eq!(generated.get(), VERIFY_CORPUS_SIZE);
                }
                assert!((1..=VERIFY_CORPUS_SIZE).contains(proof));
                verified.set(index + 1);
                Ok(())
            },
            Instant::now(),
        )
