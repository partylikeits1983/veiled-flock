...
const SOURCE_CHECKS: &[(&str, SourceCheck)] = &[
    ...
    (
        "source::no-tracked-python-or-shell-scripts",
        check_no_tracked_python_or_shell_scripts,
    ),
];
...
fn check_no_tracked_python_or_shell_scripts(root: &Path) -> Result<(), String> {
    let output = Command::new("git")
        .args(["ls-files", "--", "*.py", "*.sh"])
        .current_dir(root)
        .output()
        .map_err(|err| format!("failed to enumerate tracked scripts: {err}"))?;
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(format!("failed to enumerate tracked scripts: {stderr}"));
    }
    ...
    if scripts.trim().is_empty() {
        ...
        print!("{scripts}");
        Err("tracked Python or shell scripts found".to_owned())
    }
}
