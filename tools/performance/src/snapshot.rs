...
use crate::records::ArtifactIdentity;
...
/// Registration is performed only after the caller's frozen-source checks and
/// successful Cargo JSON artifact resolution. Publication also checks that the
/// registered executable is the process actually performing the processing.
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct ProcessorIdentity {
    pub snapshot: Snapshot,
    pub source_archive: ArtifactIdentity,
...
impl ProcessorIdentity {
    pub fn register(
        ...
        let recorded: Snapshot =
            serde_json::from_slice(&fs::read(source_directory.join("snapshot.json"))?)?;
        if serde_json::to_value(&recorded)? != serde_json::to_value(snapshot)? {
            return Err("processor source manifest differs from accepted frozen snapshot".into());
        }
        let archive: serde_json::Value =
            serde_json::from_slice(&fs::read(source_directory.join("frozen-archive.json"))?)?;
        let source_archive = ArtifactIdentity::register(&source_directory.join("frozen.tar"))?;
        if archive["snapshot_id"] != snapshot.id
            || archive["sha256"] != source_archive.sha256
            || archive["input_count"] != snapshot.inventory.len()
        {
            return Err("processor source archive does not match accepted snapshot".into());
        }
        let result = Self {
            snapshot: snapshot.clone(),
            source_archive,
            executable: ArtifactIdentity::register(executable)?,
        };
        result.verify()?;
        Ok(result)
    }
    ...
    pub fn verify(&self) -> Result<()> {
        ...
        self.source_archive.verify()?;
        self.executable.verify()?;
        Ok(())
    }

    pub fn verify_running(&self) -> Result<()> {
        self.verify()?;
        if env::current_exe()?.canonicalize()? != self.executable.path {
            return Err("publication must run the registered frozen processor executable".into());
        }
        Ok(())
    }
}
...
fn prepare_with(
    ...
    let toolchain = if toolchain_path.exists() {
        let config: toml::Value = toml::from_str(&fs::read_to_string(toolchain_path)?)?;
        Some(
            config
                .get("toolchain")
                ...
                .ok_or("frozen toolchain configuration has no channel")?
                .to_owned(),
        )
    } else {
        None
    ...
    atomic_json(
        ...
        &serde_json::json!({"external_config": external_config, "environment": build_environment, "toolchain": toolchain}),
    )?;
    seal(&root, &before)?;
...
/// Verify immutable bytes and sealing independently from the current host's
/// build environment, for read-only historical source lookup.
pub fn verify_source(snapshot: &Snapshot) -> Result<()> {
