...
use crate::{Result, sha256, xctrace};

use crate::records::SIZES;
...
fn analyze_xml(path: &Path) -> Result<xctrace::CollapseSummary> {
    Ok(xctrace::collapse(BufReader::new(File::open(path)?))?.summary)
}

pub(crate) fn validate_svg(path: &Path) -> Result<()> {
...
/// Inspect an existing capture without changing any evidence file.
pub fn analyze_capture(dir: &Path) -> Result<CaptureStats> {
    ...
    analyze_lines(
        BufReader::new(File::open(dir.join("stacks.folded"))?).lines(),
        &xml,
    )
}

pub fn analyze_resolved(trace: &xctrace::Collapsed) -> Result<CaptureStats> {
    analyze_lines(
        trace
            .stacks
            .iter()
            .map(|(stack, count)| Ok(format!("{stack} {count}"))),
        &trace.summary,
    )
}
...
fn analyze_lines(
    ...
    let stats = CaptureStats {
        ...
        xml_rows: xml.xml_rows,
        missing_or_empty_backtrace_rows: xml.xml_rows - xml.usable_rows,
...
fn source_lookup(root: &Path, snapshot: Option<&Path>, symbol: &str) -> Result<Option<String>> {
    let Some(snapshot) = snapshot else {
        return Ok(None);
    };
    // Trait-qualified Rust symbols begin with the implementing type.
    ...
    if output.exists() && fs::read_to_string(&output)? != contents {
        return Err("copied source differs from verified snapshot".into());
    }
...
/// Legacy entry point deliberately refuses in-place publication.
pub fn write_report(_root: &Path) -> Result<()> {
    Err("in-place report generation is disabled; supply a separate publication output".into())
}
...
pub(crate) fn render(
    ...
    let resume_before = if root.join("baseline-resume-before.csv").is_file() {
    ...
    md.push_str("Stacks are regenerated from verified XML frame boundaries. Embedded ASCII semicolons within symbols are displayed as `；`; original raw and demangled names are retained in each symbol map. Hotspot percentages count usable backtrace rows across threads, not wall-clock time. The original evidence is unchanged.\n\n");
    md.push_str(caveats);
    let dependency_versions: Value =
        serde_json::from_slice(&fs::read(root.join("processing/dependencies.json"))?)?;
    writeln!(
        md,
        "Stack renderer: inferno `{}` (resolved from the processor's frozen [Cargo.lock](processing/Cargo.lock)).\n",
        markdown(text(&dependency_versions, "inferno"))
    )?;
    if root.join("findings.md").is_file() {
        md.push_str(
            "\n[Authored findings](findings.md) are preserved as a separately hashed input.\n\n",
        );
    }
    writeln!(
        md,
    ...
    if let Some(snapshot) = snapshot
        && snapshot.join(&benchmark).is_file()
    {
    ...
    md.push_str("### Baseline observations and debug-symbol comparison\n\nDeltas compare the recorded normal before/after controls. Episode provenance determines whether these controls bracket any particular capture; absent episode identity does not establish uninterrupted conditions.\n\n");
    ...
    let comparison_label = if resumed { "cross-session" } else { "recorded" };
    writeln!(
        md,
    ...
    if let Some(resume_before) = &resume_before {
        md.push_str("### Resumed-episode baseline controls\n\nThese are the recorded resumed before and final after controls. Consult the episode records to determine their scope; their presence alone does not assign captures to an episode. All seven sizes were sampled in each control run; they are additional timing observations, not replacement primary captures. Exact timing and proof-size ranges: [resume before](baseline-resume-before.csv), [final after](baseline-after.csv).\n\n| Hashes | Protocol | Resume before prove / verify ms | Final after prove / verify ms | Resumed-interval change prove / verify |\n| ---: | --- | ---: | ---: | ---: |\n");
    ...
    if resumed {
        md.push_str("A continuation was recorded. Primary/repeat comparisons may span episodes; use explicit episode identities rather than case size to determine which controls apply.\n\n");
    }
    ...
    for repeat in &repeats {
        ...
        let episode = match (
            repeat.attempt["episode_id"].as_str(),
            primary.attempt["episode_id"].as_str(),
        ) {
            (Some(a), Some(b)) if a == b => "same recorded episode",
            (Some(_), Some(_)) => "different recorded episodes",
            _ => "unknown; legacy episode identity absent",
        };
        let rate = |c: &Capture| {
...
mod tests {
    ...
    #[test]
    fn canonical_xml_statistics_preserve_frames_and_recursive_inclusive_counts() -> Result<()> {
        let trace = xctrace::collapse(std::io::Cursor::new(
            r#"<trace-query-result><node><row><thread fmt="flock-verify"/><backtrace id="1"><frame id="1" name="flock_core::leaf&lt;[u8; 64]&gt;"/><frame ref="1"/><frame id="2" name="rayon_core::worker"/></backtrace></row><row><backtrace ref="1"/></row></node></trace-query-result>"#,
        ))?;
        let stats = analyze_resolved(&trace)?;
        assert_eq!(stats.total_weight, 2);
        assert_eq!(
            stats
                .inclusive
                .iter()
                .find(|h| h.symbol.contains("leaf"))
                .unwrap()
                .samples,
            2
        );
        assert_eq!(stats.self_samples.len(), 1);
        assert_eq!(stats.self_samples[0].symbol, "flock_core::leaf<[u8； 64]>");
        assert!(stats.has_verifier_worker && stats.has_rayon_worker);
        Ok(())
    }

    #[test]
    fn trait_qualified_source_lookup_and_in_place_guard() -> Result<()> {
        let p = temporary();
        let snapshot = p.join("snapshot");
        fs::create_dir_all(snapshot.join("crates/flock-core/src"))?;
        fs::write(
            snapshot.join("crates/flock-core/src/lincheck.rs"),
            "struct CscCircuit;\nimpl Trait for CscCircuit {\n fn scan(&self) {}\n}\n",
        )?;
        let output = p.join("output");
        fs::create_dir(&output)?;
        let link = source_lookup(
            &output,
            Some(&snapshot),
            "<flock_core::lincheck::CscCircuit as Trait>::scan",
        )?
        .unwrap();
        assert!(link.contains("lincheck.rs:3"));
        fs::write(output.join("report.md"), "reviewed commentary")?;
        assert!(write_report(&output).is_err());
        assert_eq!(
