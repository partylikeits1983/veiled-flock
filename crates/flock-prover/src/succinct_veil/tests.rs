...
#[test]
fn succinct_shape_rejects_unsupported_circuit_and_pcs_profiles() {
    let small = Blake3PreimageZkSetup::new(2);
    assert!(matches!(
        validate_succinct_parameters(&small.r1cs, &small.pcs_params),
        Err(SuccinctVeilError::InvalidParameters)
    ));

    let production = Blake3PreimageZkSetup::new_succinct(2);
    validate_succinct_parameters(&production.r1cs, &production.pcs_params)
        .expect("the exact production profile is pinned");
    let mut wrong_rate = production.pcs_params.clone();
    wrong_rate.log_inv_rate = 2;
    assert!(matches!(
        validate_succinct_parameters(&production.r1cs, &wrong_rate),
        Err(SuccinctVeilError::InvalidParameters)
    ));
}
...
fn mask_proof_wire_order_covers_every_observed_piop_value_once() {
    ...
    let honest_zc = ZerocheckProof {
        ...
        // `final_c_eval` is derived locally and is never observed by the
        // challenger, so it intentionally has no transcript mask.
...
#[test]
fn masking_challenger_preserves_scalar_slice_and_byte_framing() {
    let values = sequence(20, 4);
    let masks = sequence(200, 4);
    let mut actual = FsChallenger::new(b"succinct-veil-masking-framing-test");
    let mut expected = FsChallenger::new(b"succinct-veil-masking-framing-test");

    {
        let mut masking = MaskingChallenger::new(&mut actual, &masks);
        masking.observe_label(b"framing");
        masking.observe_f128(values[0]);
        masking.observe_f128_slice(&values[1..]);
        masking.observe_bytes(b"bytes");

        expected.observe_label(b"framing");
        expected.observe_f128(values[0] + masks[0]);
        expected.observe_f128_slice(
            &values[1..]
                .iter()
                .zip(&masks[1..])
                .map(|(value, mask)| *value + *mask)
        ...
        expected.observe_bytes(b"bytes");

        assert_eq!(masking.cursor, masks.len());
        assert_eq!(masking.sample_f128(), expected.sample_f128());
    }
}

#[test]
fn shifted_verifier_accepts_the_exact_committed_mask_vector() {
    ...
    // Reproduce only the domain-separated mask fork. `prove_succinct` takes
    // the witness-randomizer fork before entering `prove_succinct_veil_r1cs`.
    ...
    assert!(
        ...
        "the exact masks committed before the PIOP must satisfy the shifted verifier"
    );

    let mut verifier = FsChallenger::new(DOMAIN);
    setup
        .verify_succinct(&commitment, &proof, &digests, &mut verifier)
        .expect("the generic verifier must accept the same transcript");
}
