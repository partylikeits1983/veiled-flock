...
pub struct ProverConfig {
    ...
    /// Per-level fold-grind taper flag; length = recursive_steps + 1.
...
fn derive_ladder_shape(
    ...
    if log_n <= initial_k {
        return Err(invalid_config("log_n must be > initial_k"));
    }
    ...
    if (1usize << (n_running + rate_running)) < queries_at_rate(rate_running) {
        return Err(invalid_config(
            "L0 block_len < queries — log_n too small for chosen rate",
        ));
    }
    while n_running > 5 {
        let k = 3.min(n_running);
        ...
        loop {
            ...
            if next_rate > 20 {
                return Err(invalid_config(
                    "could not find feasible recursive rate (level too deep)",
                ));
            }
        }
    ...
    if shape.k_recursive.len() < 2 {
        return Err(invalid_config(
            "log_n too small — no recursive levels for the Ligerito recursion",
        ));
    }
...
/// Load and validate the registered security config for the effective code
/// dimension `m` and profile.
///
/// With `std`, this parses the embedded TOML source. Without `std`, it uses
/// the equivalent value generated by the build script. Callers that need a
/// prover or verifier config should prefer [`prover_config_for`] or
/// [`verifier_config_for`], which also enforce `log_batch_size`.
pub fn registered_security_config(
    m: usize,
    profile: LigeritoProfile,
...
pub fn prover_config_for(
    ...
    let sec = registered_security_config_for_batch(log_n, log_batch_size, profile)?;
...
fn registered_security_config_for_batch(
    log_n: usize,
    log_batch_size: usize,
    profile: LigeritoProfile,
) -> LigeritoConfigResult<LigeritoSecurityConfig> {
    let m = log_n + crate::pcs::LOG_PACKING;
    let sec = registered_security_config(m, profile)?;
...
impl LigeritoSecurityConfig {
    ...
    pub fn to_prover_verifier_configs(
        ...
        // Johnson levels taper one bit per round; UDR levels grind full width.
        // The flag is duplicated into prover and verifier configs.
...
fn recursive_prover_with_basis_impl<Ch: Challenger>(
    ...
    for j in 0..initial_k {
        // Each L0 fold challenge is PoW-guarded because it carries proximity risk.
        // Johnson levels taper by round; UDR levels grind full width.
...
mod tests {
    ...
    /// The public registered-config accessor matches each embedded TOML.
    #[test]
    fn registered_security_configs_match_embedded_configs() {
        for &(key, toml) in EMBEDDED_CONFIGS {
            let config = registered_security_config(key.0, key.1).unwrap_or_else(|e| {
                panic!(
                    "registered config m={} profile={} invalid: {e}",
                    key.0,
                    key.1.as_str()
                )
            });
            assert_eq!(config.to_toml_string().expect("serialize"), toml);
        }
    }
    ...
    /// Small query counts keep the path fast while exercising every feature.
    fn ood_test_configs(
        log_n: usize,
        initial_k: usize,
        ks: &[usize],
    ...
    /// Tampering with either an OOD value or fold nonce makes both reject.
    #[test]
    ...
    /// A real m22 Fast profile drives the basis opening path end to end.
    /// OOD samples and fold grinding come from the embedded TOML.
    #[test]
...
mod fold_grind_taper_tests {
    ...
    /// Under flat UDR grinding, emitted fold nonces must match charged events.
    /// This is the invariant the taper broke.
    #[test]
