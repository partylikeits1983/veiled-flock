...
impl<'a> ZkLincheckCircuit<'a> {
    ...
    pub fn new(inner: &'a dyn LincheckCircuit, layout: &'a ZkBlockLayout) -> Self {
        let pin = inner
            .const_pin_col()
...
impl LincheckCircuit for ZkLincheckCircuit<'_> {
    ...
    /// Fold the inner circuit, then add `layout`'s randomizer rows: each row adds
    /// one nonzero at its own bit and one at the pin. Prover and verifier share `layout`.
    fn fold_alpha_batched(&self, alpha: F128, eq_inner: &[F128]) -> Vec<F128> {
