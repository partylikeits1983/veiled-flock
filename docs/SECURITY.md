...
This implementation is an experimental VEIL+FLOCK candidate. It is unaudited,
unsuitable for production secrets, and is not yet supported by an end-to-end
...
The active construction is not simply "standard FLOCK plus VEIL." VEIL hides
and proves the zerocheck/lincheck verifier equations, but two additional
privacy mechanisms remain load-bearing:

1. the witness commitment and Ligerito opening use a hiding PCS variant; and
2. randomized R1CS rows are intended to hide the explicit AB and C evaluation
...
- the one-to-one order between observed transcript values, serialized fields,
  and masks;
- scalar, slice, label, and byte transcript framing; and
...
| Additive `GF(2^128)` RS code | Naive-polynomial differential test, production product/reduction tests, disjoint-domain test, and production mask-projection rank tests | Formal proof that the implemented LCH transform realizes the stated RS map |
...
The local masking argument is nevertheless elementary. Let `X` be the base
...
The intended claim is statistical zero knowledge in the classical
random-oracle model: the distance would include algebraic bad-event and oracle
prequery/collision terms. Replacing the ideal oracle by concrete SHA-256 is an
implementation assumption, not unconditional statistical zero knowledge in
the standard model.
...
The proof is not large because of a prime-field conversion: the active port
...
Until then, describe this as an experimental VEIL+FLOCK prototype and invite
review of the open obligations rather than claiming a finished ZK FLOCK.
